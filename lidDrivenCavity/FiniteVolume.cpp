...
namespace fvm
{
    ...
    FiniteMatrix::finiteMat fvm::diffusiveTerm(Fields::vectorField& vec)
    {
    ...
    FiniteMatrix::finiteMat fvm::convectionTerm(Fields::vectorField& vec, Fields::vectorField& massFluxEast, Fields::vectorField& massFluxNorth, double& blendFac)
    {
        FiniteMatrix::finiteMat APtemp(vec.size(), vector<FiniteMatrix>(vec[0].size()));
        ...
        forAllInternalUCVs(vec)
        {
            ...
            // Questo aiuta la convergenza. Visto che il termine è calcolato con le grandezze al iterazione m-1, va -con segno cambiato- a contribuire al
        ...
        cout << "CONVECTION SOURCE" << endl;
        for (int i = 1; i < vec.size() - 1; i++) {
            for (int j = 1; j < vec[1].size() - 1; j++) {
                cout << APtemp[i][j].svalue << " ";
    ...
    FiniteMatrix::finiteMat fvm::pressureGrad(const Fields::vectorField& vec, int& direction_)
    {
        ...
        cout << "PRESSURE GRAD SOURCE" << endl;
        for (int i = 1; i < vec.size()-1; i++) {
            for (int j = 1; j < vec[1].size() - 1; j++) {
                cout << APtemp[i][j].svalue << " ";
    ...
    FiniteMatrix::finiteMat fvm::HTerm(const FiniteMatrix::finiteMat& APU1, const FiniteMatrix::finiteMat& APV1, Fields::vectorField& vec, Fields::vectorField& vec2)
    {
        FiniteMatrix::finiteMat APtemp(APU1.size(), vector<FiniteMatrix>(APU1[0].size()));
    ...
    // EXTRA che nel codice stesso non serve

    FiniteMatrix::finiteMat fvm::dummyTerm(Fields::vectorField& vec)
    {
        FiniteMatrix::finiteMat APtemp(vec.size(), vector<FiniteMatrix>(vec[0].size()));
    ...
    FiniteMatrix::finiteMat fvm::divPhi(const Fields::vectorField& Feast, const Fields::vectorField& Fnorth)
    {
    ...
    FiniteMatrix::finiteMat fvm::timeDerivative(Fields::vectorField& VecNow, Fields::vectorField& VecOld, Fields::vectorField& VecOldOld, string& method)
    {

        FiniteMatrix::finiteMat APtemp(VecNow.size(), vector<FiniteMatrix>(VecNow[0].size()));
    ...
    FiniteMatrix::finiteMat fvm::convDiffusive(Fields::vectorField& vec, Fields::vectorField& massFluxEast, Fields::vectorField& massFluxNorth, double& blendFac)
    {
        FiniteMatrix::finiteMat APtemp(vec.size(), vector<FiniteMatrix>(vec[0].size()));
